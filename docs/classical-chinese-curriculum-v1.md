# Manyingo Classical Chinese Curriculum v1

## 1. 定位

Manyingo 的核心不是「12 篇指定文言篇章溫習」，而是建立**可以遷移到陌生文言篇章的語言能力**。
...
教育局的文言學與教建議亦把「掌握常見文言字詞和句式」列為理解文言篇章的基礎，並包括一詞多義、詞類活用和常見虛詞等；Manyingo v1 以此作校準，但不綁定單一課本。
