...
function practiceSignature(item){return [String(item&&item.skillId||''),String(item&&item.routeKpId||item&&item.kpId||''),String(item&&item.kpId||''),String(item&&item.completedAt||''),String(item&&item.strategy||'legacy'),Number(item&&item.beforeMastery)||0,Number(item&&item.afterMastery)||0].join('|')}
...
async function syncNow(options){if(syncPromise)return syncPromise;syncPromise=(async()=>{const silent=options&&options.silent;try{if(!silent)emit({status:'syncing'});const fb=await firebase(),uid=await fb.ensureLogin();if(!uid)return{ok:false,reason:'auth'};let local=parse(LEARNING_KEY,{}),rotation=parse(ROTATION_KEY,{});const [snapshot,knowledgeState,skillState,conceptState,game]=await Promise.all([fb.fetchClientSyncState(uid),fb.fetchUserKnowledgeState(uid),fb.fetchUserSkillState(uid),fb.fetchUserConceptState(uid),fb.fetchUserGamification(uid)]);if(snapshot&&snapshot.learningState)local=mergeLearningState(local,snapshot.learningState);local=mergeLearningState(local,{skillMastery:skillState||{}});if(snapshot&&snapshot.questionRotation)rotation=mergeRotation(rotation,snapshot.questionRotation);write(LEARNING_KEY,local);write(ROTATION_KEY,rotation);const engine=learning();if(engine){Object.entries(knowledgeState||{}).forEach(([kpId,record])=>{const current=engine.getKnowledge(kpId)||{};if(remoteKnowledgeWins(current,record)&&typeof engine.syncRemoteResult==='function')engine.syncRemoteResult(kpId,record)});if(typeof engine.syncRemoteSkillState==='function')engine.syncRemoteSkillState(skillState||{});if(typeof engine.syncRemoteConceptState==='function')engine.syncRemoteConceptState(conceptState||{});if(game&&typeof engine.syncGamification==='function')engine.syncGamification(mergeGame(engine.getProgress?engine.getProgress():local,game));local=parse(LEARNING_KEY,local)}local=mergeLearningState(local,snapshot&&snapshot.learningState);local=mergeLearningState(local,{skillMastery:skillState||{}});rotation=mergeRotation(parse(ROTATION_KEY,rotation),snapshot&&snapshot.questionRotation);write(LEARNING_KEY,local);write(ROTATION_KEY,rotation);const saved=await fb.saveClientSyncState(uid,{learningState:local,questionRotation:rotation});emit({status:saved?'synced':'local-only',uid});return{ok:!!saved,uid,learningState:local,questionRotation:rotation}}catch(error){emit({status:'error',error:String(error&&error.message||error)});return{ok:false,error}}finally{syncPromise=null}})();return syncPromise}
...
const api={LEARNING_KEY,ROTATION_KEY,SENTENCE_HISTORY_KEY,MAX_ROTATION_PER_KP,MAX_SENTENCE_HISTORY,timeValue,recordTime,newerRecord,newerSkillRecord,practiceSignature,mergePracticeHistory,mergeLearningState,mergeRotation,mergeGame,syncNow,schedule,clearLocal,install};
