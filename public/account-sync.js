...
function newerSkillRecord(a,b){const left=a&&typeof a==='object'?a:{},right=b&&typeof b==='object'?b:{},lt=recordTime(left),rt=recordTime(right);if(lt!==rt)return rt>lt?right:left;const ln=String(left.source||'').includes('server-native'),rn=String(right.source||'').includes('server-native');if(ln!==rn)return rn?right:left;const la=Number(left.attempts)||0,ra=Number(right.attempts)||0;if(la!==ra)return ra>la?right:left;return left}
