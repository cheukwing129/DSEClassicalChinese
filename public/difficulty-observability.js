...
function studentSummary(decision){const item=decision&&typeof decision==='object'?decision:{},selection=String(item.selectionReason||'target-tier'),reason=item.reasonLabel||reasonLabel(item.reason),extra=selection!=='target-tier'?(item.selectionLabel||selectionLabel(selection)):'';return{tier:tierLabel(item.selectedTier),reason:extra?reason+'；'+extra:reason,evidence:evidenceText(item),adjustment:adjustmentText(item)}}
