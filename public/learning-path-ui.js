...
content.getKnowledgePointIds=function(options){
 ...
 return path.availableKnowledgePointIds(stages());
};
function render(){
...
function install(){
 if(!document.getElementById('learningPath')){
   ...
   if(plan){
     const card=document.createElement('div');card.className='card';card.id='learningPath';plan.parentNode.insertBefore(card,plan);
   }
 }
