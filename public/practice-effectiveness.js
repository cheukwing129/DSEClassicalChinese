...
function annotateLatest(entry,strategy){if(!entry)return entry;const normalized=strategy==='remedial'?'remedial':'targeted',store=storage();try{const data=readState(),history=Array.isArray(data.practiceHistory)?data.practiceHistory:[],index=history.findIndex(item=>String(item&&item.kpId)===String(entry.kpId)&&String(item&&item.completedAt)===String(entry.completedAt));if(index>=0&&store){history[index]={...history[index],strategy:normalized};data.practiceHistory=history;store.setItem(KEY,JSON.stringify(data))}}catch(e){}return{...entry,strategy:normalized}}
function install(){const learning=engine();if(!learning||learning.__practiceEffectivenessInstalled)return false;const originalRecord=typeof learning.recordPracticeSession==='function'?learning.recordPracticeSession.bind(learning):null;if(originalRecord){learning.recordPracticeSession=function(session){const entry=originalRecord(session);return annotateLatest(entry,String(session&&session.strategy||'').toLowerCase()==='remedial'?'remedial':'targeted')}}learning.getRemediationStatus=function(kpId){const history=typeof learning.getPracticeHistory==='function'?learning.getPracticeHistory({kpId,days:7,limit:50}):[],record=typeof learning.getKnowledge==='function'?learning.getKnowledge(kpId):{},result=evaluate(history,{currentMastery:Number(record&&record.mastery)||0});return{kpId:String(kpId),...result}};learning.__practiceEffectivenessInstalled=true;return true}
