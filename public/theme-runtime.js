...
function loadStage3Diagnostics(){if(typeof document==='undefined'||root.ManjingoStage3Diagnostics||document.getElementById('stage3DiagnosticsScript'))return false;const pathname=String(root.location&&root.location.pathname||''),page=pathname.split('/').pop();if(page&&page!=='index.html')return false;const script=document.createElement('script');script.id='stage3DiagnosticsScript';script.src='./stage3-diagnostics.js';script.async=true;document.head.appendChild(script);return true}
function install(){apply(read(),{persist:false});buildSwitcher();loadStage3Diagnostics();return true}
...
return{storageKey:STORAGE_KEY,themes:THEMES,normalizeTheme,read,apply,buildSwitcher,loadStage3Diagnostics,install};
});
